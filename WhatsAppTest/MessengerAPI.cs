...
namespace WhatsAppTest
{
    public class MessengerAPI
    {
        ...
        /// <returns></returns>
        public List<string> SendMessage(string phoneNumber, string message, bool useLine)
        {
            ...
            try
            {
                Message result = service.SendMessage(phoneNumber, message);
            }
            catch (Exception e)
            {
                Debug.WriteLine(e.Message);
                rejectedNumbers.Add(phoneNumber);
            }
